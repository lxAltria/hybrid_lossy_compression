...
int old_estimate_zfp_sz_compress_num(float * data, int r1, int r2, int r3, double abs_eb, int zfp_factor){
	...
	size_t ny = r2 / bs;
	size_t nz = r3 / bs;
	// int sample_num = (MIN(nx, MIN(ny, nz))) >> 1;
	int sample_num = MIN(ny, nz);
	sample_num = MIN(sample_num, nx);
	int sample_nx = nx / sample_num;
	int sample_ny = ny / sample_num;
	int sample_nz = nz / sample_num;
	int sample_block_size = sample_num;
	sample_num >>= 1;
	int half_sample_num = sample_num >> 1;
	...
	signed int block_data_fp[64] = {0};
	signed int ordered_fp[64] = {0};
	float block_data[64] = {0};
	for(int ix=0; ix<sample_nx; ix++){
		for(int iy=0; iy<sample_ny; iy++){
			for(int iz=0; iz<sample_nz; iz++){
				float * sample_block_data = data + ix*bs*sample_block_size*r2*r3 + iy*bs*sample_block_size*r3 + iz*bs*sample_block_size;
				for(int sample=0; sample<half_sample_num; sample++){
		    		for(int order=0; order<8; order++){
		    			// ~sample = sample_num - sample, sample the following blocks 
		    			...
		    			// [~sample, ~sample, sample]
		    			// [~sample, ~sample, ~sample]
		    			int offset_x = ((order>>2)&1) ? 2*(sample_num - 1 - sample) : 2*sample;
		    			int offset_y = ((order>>1)&1) ? 2*(sample_num - 1 - sample) : 2*sample;
		    			int offset_z = (order&1) ? 2*(sample_num - 1 - sample) : 2*sample;
		    			float * block_data_pos = sample_block_data + offset_x*bs*r2*r3 + offset_y*bs*r3 + offset_z*bs;
			    		// for each sample, sample 2x2x2 bsxbsxbs blocks
						for(int i=0; i<2; i++){
							for(int j=0; j<2; j++){
								for(int k=0; k<2; k++){
									float * data_pos = block_data_pos + i*bs*r2*r3 + j*bs*r3 + k*bs;
									float max_ele = 0;
									for(int ii=0; ii<4; ii++){
										for(int jj=0; jj<4; jj++){
											for(int kk=0; kk<4; kk++){
												float cur_data = *(data_pos + ii*r2*r3 + jj*r3 + kk);
									...
									for(int num=0; num<64; num++){
								    	...
								    	sample_coeff[num][i][j][k] = ldexp((float)fp, intbits - wlen + e);
									}
									...
									if(k){
										...
										uint intprec = 32;
										uint kmin = intprec > maxprec ? intprec - maxprec : 0;
										signed int * block_data_fp_pos = ordered_fp;
										for(int group=0; group<9; group++){
											...
											if(group == 8){
												// last group
												skip_bits += intprec - kmin - max_ele;
											}
										}
									}
						...
						for(int ii=0; ii<2; ii++){
							for(int jj=0; jj<2; jj++){
								...
								for(int group=0; group<9; group++){
									for(int e=0; e<gp_element_num[group]; e++){
										// float pred = sample_coeff[index][0][1][1] + sample_coeff[index][1][0][1] + sample_coeff[index][1][1][0] -
										// 	sample_coeff[index][0][0][1] - sample_coeff[index][0][1][0] - sample_coeff[index][1][0][0] + sample_coeff[index][0][0][0];
										// double diff = sample_coeff[index][1][1][1] - pred;
	...
	float aver_zfp_br = 0;
	for(int i=0; i<9; i++){
		zfp_br[i] = encoding_length[i]*1.0/total_sample_count + group_bits[i]*1.0/total_sample_count/64 + sign_bits[i]*1.0/total_sample_count/64 + skip_bits*1.0/total_sample_count/64;
...
void old_zfp_sample_compress(float * data, size_t r1, size_t r2, size_t r3, double abs_eb, int zfp_factor, int * zfp_sz_compress_num, double * mse, float * bit_rate){
	int block_size = 8;
	...
	int half_sample_num = sample_block_size >> 1;

	int sz_compress_num = old_estimate_zfp_sz_compress_num(data, r1, r2, r3, abs_eb, zfp_factor);
	// sz_compress_num = 0;
    ...
    int * combined_zfp_exp = (int *) malloc(num_sample_blocks * 8 * sizeof(int));
	...
	double final_mse = 0;
	for(size_t ix=0; ix<sample_nx; ix++){
		for(size_t iy=0; iy<sample_ny; iy++){
			for(size_t iz=0; iz<sample_nz; iz++){
				float * sample_block_data = data + ix*block_size*sample_block_size*r2*r3 + iy*block_size*sample_block_size*r3 + iz*block_size*sample_block_size;
				for(int sample=0; sample<half_sample_num; sample++){
		    		for(int order=0; order<8; order++){
						int offset_x = ((order>>2)&1) ? (sample_block_size - 1 - sample) : sample;
		    			int offset_y = ((order>>1)&1) ? (sample_block_size - 1 - sample) : sample;
		    			int offset_z = (order&1) ? (sample_block_size - 1 - sample) : sample;
		    			float * block_data_pos = sample_block_data + offset_x*block_size*r2*r3 + offset_y*block_size*r3 + offset_z*block_size;
						// compress by mZFP
						// each block includes 8 ZFP blocks
						for(int z=0; z<8; z++){
							float * zfp_data_pos = block_data_pos + zfp_offsets[zfp_order[z]];
							// encoding
							float max_ele = 0;
							for(int ii=0; ii<4; ii++){
								for(int jj=0; jj<4; jj++){
	...
	SZ_Init(NULL);
	{
		*((float *) comp_data_pos) = zfp_factor;
		...
		unsigned char * compressed_exp = SZ_compress_args(SZ_INT32, combined_zfp_exp, &compressed_exp_size, ABS, 0.5, 0, 0, 0, 0, 0, 0, num_sample_blocks*8);
...
unsigned char * compress_block(float * data, size_t r3, size_t r2, size_t r1, double eb, size_t * out_size, size_t * out_size_before_lossless, int * select){
	...
	int best_zfp_factor = zfp_factor;
	float min_bit_rate = 32;
	float best_mse = mse[1];
	float best_bit_rate = bit_rates[1];
	int best_pos = 1;
	best_zfp_factor = 2;
	sz_compress_num = sample_sz_compress_num[1];
	for(int i=2; i<ZFP_FACTOR_SAMPLE_NUM; i++){
		float aver_bit_rate_decay = bit_rates[0] / best_bit_rate;
		float aver_mse_increase = best_mse / mse[0];
		float cur_bit_rate_decay = best_bit_rate / bit_rates[i];
		float bit_rate_decay_degree = log2(cur_bit_rate_decay) / log2(aver_bit_rate_decay);
		float cur_mse_increase = mse[i] / best_mse;
		float mse_increase_degree = log2(cur_mse_increase) / log2(aver_mse_increase);
		if(bit_rate_decay_degree > 0.9 * mse_increase_degree){
			best_bit_rate = bit_rates[i];
			best_mse = mse[i];
			best_zfp_factor = 1 << i;
			sz_compress_num = sample_sz_compress_num[i];
		}
	}
	float zfp_psnr = 20*log10(value_range)-10*log10(best_mse);
	float zfp_bit_rate = best_bit_rate;
